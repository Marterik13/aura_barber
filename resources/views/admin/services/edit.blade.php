<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl gold-text leading-tight">
            {{ __('Editar Servicio: ') }} {{ $service->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-[#1A1A1A] border border-gray-800 overflow-hidden shadow-2xl sm:rounded-3xl">
                <div class="p-12 md:p-16">
                                     <x-admin-header title="Editar Servicio" :breadcrumbs="[
    ...
    ['name' => 'Editar']
                ...
                <form action="{{ route('admin.services.update', $service) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <x-wire-input label="Precio" name="price" 
                                :value="old('price', $service->price)" 
                                type="number" step="0.01" placeholder="Ej. 15.00" required />
                        </div>

                        <div>
                            <x-wire-input label="Duración (minutos)" name="duration" 
                                :value="old('duration', $service->duration)" 
                                type="number" placeholder="Ej. 30" required />
                        </div>

                        <div class="md:col-span-2">
                            <x-wire-textarea label="Descripción" name="description" placeholder="Breve descripción del servicio">{{ old('description', $service->description) }}</x-wire-textarea>
                        </div>
